...
### Running the site
To make running the website as easy as possible we have created a docker setup with a `docker-compose` file. All you need to do to get the site running along with a database is to run `docker compose up -d --build`. This command should both start a database (or run the container from previous runs of the `docker-compose` file) and then the website. When starting the website, docker will first run commands to initialize the database, and then populate it with some necessary data to get the website working.
